...
function validateCard(arr) {
  ...
  const sumArr = digits.reduce( (accum, val) => accum+val);
...
function findInvalidCards(nestedArr) {
  ...
  for(let i=0; i<nestedArr.length; i++) {
    let flag = validateCard(nestedArr[i]);
    if(!flag){
...
// const testArr = findInvalidCards(batch);
// console.log(testArr);

// const comp = idInvalidCardCompanies(testArr);
// console.log(comp);
